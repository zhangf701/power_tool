...
For renewable CSV fields, target-column selection, future-weather forecast CSV input, solar-geometry correction, and algorithm details, see `PowerTool_Forecasting_Algorithms_and_Data_Format.md`.

## Data

- Bundled offline examples/schema samples: `CAISO_RENEWABLE_SAMPLE`, `NREL_SOLAR_WIND_SAMPLE`, and `BAIDU_KDD_SDWPF_WIND_SAMPLE`.
- Training CSV imports may include `solar_mw`, `wind_mw`, `ghi_wm2`, `wind_speed_mps`, `temperature_c`, `Wspd`, and `Patv`.
- A separate future-weather forecast CSV can be imported without `solar_mw` or `wind_mw`. It needs a time column and at least one of `temperature_c`, `ghi_wm2`, `wind_speed_mps`, or `cloud_cover_pct`.
...
The default model path uses a lightweight adaptive ensemble together with date, hour, hemisphere-aware season, latitude, longitude, altitude, an automatically inferred climate block, GHI, and wind-speed patterns. Optional scikit-learn tree models are imported lazily only when selected. Missing weather first uses timestamp-matched or interpolated future-weather CSV values; uncovered fields then fall back to historical same-hour patterns and geography/climate baselines. If a capacity limit is entered, forecasts are clipped to `[0, capacity]`. For solar/PV resources, post-processing enforces zero output whenever the solar altitude angle is below 0°, instead of relying on the model to learn sunset behavior. Built-in holidays cover China (CN) and the United States (US); other calendars can be added through `data/forecast_holidays.json` or a configured calendar path.
